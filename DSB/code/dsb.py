...
The complexity of the benchmark can be determined by
    ...
    - curviness: number of extremes within base interval 
...
def generate_sine_fcts_for_one_dimension(n_data, desired_curv, desired_med_vel,
    ...
    curr_curv = compute_curviness_empirically(final_base_vals)
    ...
    min_vel, max_vel, med_vel, _ = compute_velocity_empirically(final_vals)
...
def compute_single_curviness(a, b, c, time, n_base_time_points):
    ...
    single_curv = compute_curviness_empirically(single_base_vals)
    return single_curv


def compute_composition_curviness(fcts, time, n_base_time_points):
    '''
    Computes curvines of composition function empirically.
    '''
    compos_vals = compute_vals_for_fct(fcts, time)
    compos_base_vals = compos_vals[:n_base_time_points]
    return compute_curviness_empirically(compos_base_vals)


def compute_curviness_empirically(values):
    '''
...
def compute_velocity_empirically(values):
...
def correct_velocity(fcts, time, desired_med_vel, scaling_idx):
    ...
    _, _, med_vel, _ = compute_velocity_empirically(values)
    ...
    _, _, curr_med, _ = compute_velocity_empirically(corrected_values)
    # test
